...
#*************
# Required (runtime)
#libisoburn (or any provider of xorrisofs)
#
# Options
#********
...
DL_URL=http://fy.chalmers.se/~appro/linux/DVD+RW/tools
DL_ALT=
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
sed -i '/stdlib/a #include <limits.h>' ${SRCDIR}/transport.hxx
sed -i 's#mkisofs"#xorrisofs"#' ${SRCDIR}/growisofs.c
sed -i 's#mkisofs#xorrisofs#;s#MKISOFS#XORRISOFS#' ${SRCDIR}/growisofs.1
#
# Build
#^^^^^^^
${MAKE} ${MAKE_FLAGS}
...
#
as_root install -v -m644 -D ${SRCDIR}/index.html \
        ${DOCDER}/index.html
