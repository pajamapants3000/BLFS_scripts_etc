...
# Options
#********
# Download optional documentation
DOC=1
#
# Preparation
#*************
...
DL_URL=http://www.nasm.us/pub/nasm/releasebuilds
DL_ALT=
REPO=
# VCS=[git,hg,svn,...]
#VCS=${VERSION}
...
#
if ((${DOC})) && ! [ -f ${PROG}-${VERSION}-xdoc.${ARCHIVE} ]; then
    wget ${DL_URL}/${VERSION}/${PROG}-${VERSION}-xdoc.${ARCHIVE} \
        -O ${PROG}-${VERSION}-xdoc.${ARCHIVE}
fi
...
#
if ((${DOC})); then
    tar -xf ../${PROG}-${VERSION}-xdoc.${ARCHIVE} --strip-components=1
fi
##autogen first
#./autogen.sh
#
...
#
if ((${DOC})); then
    as_root install -m755 -d         /usr/share/doc/${PROG}-${VERSION}/html
    as_root cp -v doc/html/*.html    /usr/share/doc/${PROG}-${VERSION}/html
    as_root cp -v doc/*.{txt,ps,pdf} /usr/share/doc/${PROG}-${VERSION}
    as_root cp -v doc/info/*         /usr/share/info
    as_root install-info /usr/share/info/nasm.info /usr/share/info/dir
fi
#
popd
