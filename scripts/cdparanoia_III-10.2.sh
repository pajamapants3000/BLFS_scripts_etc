...
DL_URL=http://downloads.xiph.org/releases
DL_ALT=
DL_PATCH=http://www.linuxfromscratch.org/patches/blfs/svn/cdparanoia-III-10.2-gcc_fixes-1.patch
...
#
    ...
    if ! [ -f ${PROG}-${VERSION}.${ARCHIVE} ]; then
        wget ${DL_URL}/cdparanoia/${PROG_ALT}-${VERSION}.src.${ARCHIVE} \
...
#
    ...
    if ! [ -f cdparanoia-III-10.2-gcc_fixes-1.patch ]; then
        wget ${DL_PATCH} -O ${WORKING_DIR}/cdparanoia-III-10.2-gcc_fixes-1.patch
    fi
    # Verify package
    #****************
    if [ "${SHASUM}" ]; then
