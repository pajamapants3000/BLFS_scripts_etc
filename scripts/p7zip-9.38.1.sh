...
DL_ALT=
MD5=6cba8402ccab2370d3b70c5e28b3d651 
SHASUM=
SHAALG=1
REPO=
...
TEST_FLAGS="-k"
INSTALL="install"
INSTALL_FLAGS="DEST_HOME=${PREFICKS} DEST_MAN=${MANDER} DEST_SHARE_DOC=${DOCDER}"
#
...
if [ "${VCS}" ]; then
    ...
    if ! [ -f ${PROG}-${VERSION}.${ARCHIVE} ]; then
        wget ${DL_URL}/${PROG}/${PROG}/${VERSION}/${PROG}_${VERSION}_src_all.${ARCHIVE} \
