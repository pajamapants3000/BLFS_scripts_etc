...
# Options
#********
# Build optional documentation if necessary dependencies are present
OPT_DOCS=1
# Uncomment if this system uses systemd
#HAS_SYSTEMD=1
...
#TREATASNEW=1
#TREATASOLD=1
# Optional dependecies installed?
if (cat /list-${CHRISTENED}-${SURNAME} | grep "^linux_pam" > /dev/null); then
    HAS_LPAM=1
fi
if (cat /list-${CHRISTENED}-${SURNAME} | grep "^polkit" > /dev/null); then
    HAS_POLK=1
fi
# Build optional documentation if xmlto is present
if (cat /list-${CHRISTENED}-${SURNAME} | grep "^xmlto" > /dev/null); then
    HAS_XMLT=1
fi
...
DL_URL=https://github.com/Consolekit2/ConsoleKit2/releases/download
DL_ALT=
MD5=fcdaf885658f6b22e5f4930cc63fc3ea
SHASUM=
SHAALG=1
REPO=
...
XINITRCDIR=/etc/X11/app-defaults/xinitrc.d
...
CONFIG_FLAGS="--enable-udev-acl --with-xinitrc-dir=${XINITRCDIR}"
((HAS_SYSTEMD)) || CONFIG_FLAGS="$CONFIG_FLAGS --with-systemdsystemunitdir=no"
((HAS_LPAM)) && CONFIG_FLAGS="$CONFIG_FLAGS --enable-pam-module" || (exit 0)
((HAS_POLK)) && CONFIG_FLAGS="$CONFIG_FLAGS --enable-polkit"     || (exit 0)
((OPT_DOCS)) && ((HAS_XMLT)) &&
        CONFIG_FLAGS="$CONFIG_FLAGS --enable-docbook-docs"       || (exit 0)
MAKE_FLAGS=""
TEST=
TEST_FLAGS="-k"
INSTALL="install"
...
#
as_root mv -v ${XINITRCDIR}/90-consolekit{,.sh}
as_root cp -v \
${BLFSDIR}/files/usr/lib/ConsoleKit/run-session.d/pam-foreground-compat.ck \
...
as_root tee -a /etc/pam.d/system-session << "EOF"
