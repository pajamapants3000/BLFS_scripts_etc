...
# Comment out to build ALL help languages
HELP_LANGS="en"
# Uncomment to build ALL help languages
#ALL_HELP_LANGS=1
...
DL_URL=http://download.gimp.org/pub/gimp/help
MD5=a591c8974b2f4f584d0a769d52ed6c5b
SHASUM=
SHAALG=1
REPO=
...
if [ "${CONFIGURE}" ]; then
    if [ ${CMAKE_GEN} ]; then
        ...
        if ((ALL_HELP_LANGS)); then
            ${CONFIGURE} ${CONFIG_FLAGS}
        else
            ALL_LINGUAS=${HELP_LANGS} ${CONFIGURE} ${CONFIG_FLAGS}
        fi
    fi
fi
#
...
#
as_root chown -R root:root ${PREFICKS}/share/gimp/2.0/help
