...
#links-2.12
#elinks
#openldap-2.4.44 (only if apr_util installed with openldap support)
#rsync-3.1.2
#distcache
#lua-5.3.2
#
...
DL_URL=https://archive.apache.org/dist
DL_ALT=
MD5=3690b3cc991b7dfd22aea9e1264a11b9
SHASUM=
SHAALG=1
REPO=
...
PATCH_REQ=httpd-2.4.18-blfs_layout-1.patch
DL_PATCH=http://www.linuxfromscratch.org/patches/blfs/svn/${PATCH_REQ}
# *** The patch covers all directory structuring! DO NOT SET ANY PATHS ***
...
CONFIG_FLAGS="${CONFIG_FLAGS} --enable-authnz-fcgi"
CONFIG_FLAGS="${CONFIG_FLAGS} --enable-layout=BLFS"
CONFIG_FLAGS="${CONFIG_FLAGS} --enable-mpms-shared=all"
CONFIG_FLAGS="${CONFIG_FLAGS} --enable-suexec=shared"
CONFIG_FLAGS="${CONFIG_FLAGS} --with-apr=/usr/bin/apr-1-config"
CONFIG_FLAGS="${CONFIG_FLAGS} --with-apr-util=/usr/bin/apu-1-config"
CONFIG_FLAGS="${CONFIG_FLAGS} --with-suexec-bin=/usr/lib/httpd/suexec"
CONFIG_FLAGS="${CONFIG_FLAGS} --with-suexec-caller=apache"
CONFIG_FLAGS="${CONFIG_FLAGS} --with-suexec-docroot=/srv/www"
CONFIG_FLAGS="${CONFIG_FLAGS} --with-suexec-logfile=/var/log/httpd/suexec.log"
CONFIG_FLAGS="${CONFIG_FLAGS} --with-suexec-uidmin=100"
CONFIG_FLAGS="${CONFIG_FLAGS} --with-suexec-userdir=public_html"
# TODO: how do I put spaces in these things!? Adding this one manually
#CONFIG_FLAGS="${CONFIG_FLAGS} --enable-mods-shared='all cgi'"
MAKE_FLAGS=""
TEST=
TEST_FLAGS="-k"
INSTALL="install"
...
if [ "${PROGGROUP}" ]; then
    ...
    if [ "${PROGUSER}" ]; then
        ...
        as_root useradd -c "${USRCMNT}" -d /srv/www \
...
#
    # Download patch
    #****************
    wget ${DL_PATCH} -O ${WORKING_DIR}/${PATCH_REQ}
    # Extract package
    #*****************
    mkdir -v ${PKGDIR}
...
#
patch -Np1 < ${WORKING_DIR}/${PATCH_REQ}
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
sed '/dir.*CFG_PREFIX/s@^@#@'     -i ${SRCDIR}/support/apxs.in
#
# Configure
#^^^^^^^^^^^
if [ "${CONFIGURE}" ]; then
    if [ ${CMAKE_GEN} ]; then
        ...
        ${CONFIGURE} ${CONFIG_FLAGS} --enable-mods-shared='all cgi'
    fi
fi
#
