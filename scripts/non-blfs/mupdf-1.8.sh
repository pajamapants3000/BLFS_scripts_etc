...
# Dependencies
#*************
# This is weird... mupdf USES these thrid party libraries but they seem
#+to be included. Not entirely sure but easy enough to get them if needed.
# Required
#freetype-2.5.5
#jpeg-9.0
#openjpeg-2.0.0 (with patches)
#zlib-1.2.7
# Optional
#curl-7.31.0
# Additional - included in package
#jbig2dec
#mujs
#
# Preparation
...
DL_URL=http://mupdf.com/downloads
DL_ALT=
MD5=3205256d78d8524d67dd2a47c7a345fa
SHASUM=
SHAALG=1
REPO=
...
#
    ...
    if ! [ -f ${WORKING_DIR}/${PROG}-${VERSION}.${ARCHIVE} ]; then
        wget ${DL_URL}/${PROG_ALT}-${VERSION}-source.${ARCHIVE} \
            -O ${WORKING_DIR}/${PROG}-${VERSION}.${ARCHIVE} || FAIL_DL=1
        # FTP/alt Download:
        if (($FAIL_DL)) && [ "$DL_ALT" ]; then
            wget ${DL_ALT}/${PROG_ALT}-${VERSION}-source.${ARCHIVE} \
