...
[[ -d ${XDG_CONFIG_HOME}/vifm ]] &&
    mv ${XDG_CONFIG_HOME}/vifm ${XDG_CONFIG_HOME}/vifm.bak || (exit 0)
[[ -d ${XDG_CONFIG_HOME} ]] || mkdir -pv ${XDG_CONFIG_HOME}
cp -R ${BLFSDIR}/files/home/.config/vifm ${XDG_CONFIG_HOME}/
tee -a ${XDG_CONFIG_HOME}/bash/bash_envar << EOF

# Begin vifm additions
VIFM=${XDG_CONFIG_HOME}/vifm
MYVIFMRC=${VIFM}/vifmrc
export VIFM MYVIFMRC
# End vifm additions

EOF
#
# You may want to update lincon after this so that vifmrc is linked to it.
###################################################
