...
DOC=1
# Uncomment to build support for parallel ports
PARA=1
# Uncomment to build support for Linux sysfs GPIO interface
GPIO=1
...
DL_URL=http://download.savannah.gnu.org/releases/avrdude/
DL_ALT=
MD5=58bb42049122cf80fe4f4d0ce36d92ee
SHASUM=
SHAALG=1
REPO=
...
CONFIG_FLAGS=""
if ((DOC)) &&
    (cat /list-${CHRISTENED}-${SURNAME} | grep '^texinfo-' > /dev/null);
then
    CONFIG_FLAGS="${CONFIG_FLAGS} --enable-doc"
else
    CONFIG_FLAGS="${CONFIG_FLAGS} --disable-doc"
fi
if ((PARA)); then
    CONFIG_FLAGS="${CONFIG_FLAGS} --enable-parport"
else
    CONFIG_FLAGS="${CONFIG_FLAGS} --disable-parport"
fi
if ((DOC)); then
    CONFIG_FLAGS="${CONFIG_FLAGS} --enable-linuxgpio"
else
    CONFIG_FLAGS="${CONFIG_FLAGS} --disable-linuxgpio"
fi
MAKE_FLAGS=""
TEST=
TEST_FLAGS="-k"
