...
DL_ALT=
MD5=3cd7c01dedcfd41ad9e5a445dcaee3ad
SHASUM=
SHAALG=1
REPO=
...
# node configure does not have the standard path options: just prefix
PREFICKS=/usr
#SYSCONFDER=/etc
...
CONFIG_FLAGS="--with-intl=system-icu"
MAKE_FLAGS=""
TEST=test
TEST_FLAGS="-k"
INSTALL="install"
...
#
    ...
    if ! [ -f ${WORKING_DIR}/${PROG}-${VERSION}.${ARCHIVE} ]; then
        wget ${DL_URL}/v${VERSION}/${PROG_ALT}-v${VERSION}.${ARCHIVE} \
            -O ${WORKING_DIR}/${PROG}-${VERSION}.${ARCHIVE} || FAIL_DL=1
        # FTP/alt Download:
        if (($FAIL_DL)) && [ "$DL_ALT" ]; then
            wget ${DL_ALT}/v${VERSION}/${PROG_ALT}-v${VERSION}.${ARCHIVE} \
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
sed -i "s/\(linux_use_gold_flags%':\) 1/\1 0/" deps/v8/build/toolchain.gypi
#
# Configure
#^^^^^^^^^^^
if [ "${CONFIGURE}" ]; then
...
#^^^^^^^
${MAKE} ${MAKE_FLAGS}
((DOC)) && ${MAKE} doc || (exit 0)  # 'if' would catch errors in make doc ...?
#
#
...
((DOC)) &&
        ...
        doc/node.1 /usr/share/man/man1/node.1 || (exit 0)
#
