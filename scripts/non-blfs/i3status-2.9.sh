#!/bin/bash -ev
# Installation script for i3status-2.9
# Updated 07/19/2015
#
...
PROCEED="yes"
grep i3status-2.9 /list-$CHRISTENED"-"$SURNAME > /dev/null && ((\!$?)) &&\
    echo "Previous installation detected, proceed?" && read PROCEED
[ $PROCEED = "yes" ] || [ $PROCEED = "y" ] || exit 0
# Download:
wget http://i3wm.org/i3status/i3status-2.9.tar.bz2
# md5sum:
#
##
tar -xvf i3status-2.9.tar.bz2
cd i3status-2.9
patch -p1 < ../patches/i3status-2.9-patch.diff
source ../blfs_profile
...
pathremove /usr/sbin
cd ..
as_root rm -rf i3status-2.9
# Add to list of installed programs on this system
echo "i3status-2.9" >> /list-$CHRISTENED"-"$SURNAME
#
