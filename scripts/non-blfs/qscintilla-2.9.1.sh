...
#source loadqt5 (we'll get this later since either Qt can be used)
...
DL_URL=http://sourceforge.net/projects/pyqt/files
DL_ALT=
MD5=38e6248cb970adf6d05aea7d94f1288e
SHASUM=
SHAALG=1
REPO=
...
((STATIC)) && patch_append="-static" || patch_append=
PATCH=${PROG}-${VERSION}${patch_append}.patch
if [ ${PATCH} ]; then
    [ -f ${PATCHDIR}/${PATCH} ] ||
...
#
    ...
    if ! [ -f ${WORKING_DIR}/${PROG}-${VERSION}.${ARCHIVE} ]; then
        wget ${DL_URL}/${PROG_ALT}2/${PROG_ALT}-${VERSION}/${PROG_ALT}-gpl-${VERSION}.${ARCHIVE} \
