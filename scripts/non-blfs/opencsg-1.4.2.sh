...
DL_URL=http://www.opencsg.org
DL_ALT=
MD5=3e909f2f9665e961cc32bbbc2390bbec
SHASUM=
SHAALG=1
REPO=
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
for file in "opencsg" "src/src" "example/example"; do
    sed -i "s@/usr/local@${PREFICKS}@g" ${file}.pro
done
#
# Configure
#^^^^^^^^^^^
