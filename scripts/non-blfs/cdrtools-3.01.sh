...
DL_ALT=
MD5=7d45c5b7e1f78d85d1583b361aee6e8b
SHASUM=62766aa9bfeec91b3c78da7c59b5c96bfbec84d9
SHAALG=1
REPO=
...
#
# Default make flags (emptied for this package)
#********************
...
#
    ...
    if ! [ -f ${PROG}-${VERSION}.${ARCHIVE} ]; then
        wget ${DL_URL}/${PROG}/files/${PROG}-${VERSION}.${ARCHIVE} \
            -O ${WORKING_DIR}/${PROG}-${VERSION}.${ARCHIVE} || FAIL_DL=1
        # FTP/alt Download:
        if (($FAIL_DL)) && [ "$DL_ALT" ]; then
            wget ${DL_ALT}/${PROG}/files/${PROG}-${VERSION}.${ARCHIVE} \
...
echo "Adding environment profile ..."
as_root install -Dm644 -o root -g root \
    ${BLFSDIR}/files/etc/profile.d/cdrtools.sh /etc/profile.d/
echo "Adding libraries to ld.so.conf ..."
as_root tee -a /etc/ld.so.conf << EOF
# Begin Cdrtools additions
/opt/schily/lib
# End Cdrtools additions
