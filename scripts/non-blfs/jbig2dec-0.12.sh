...
DL_ALT=
MD5=b73cfb55c61c6978e6539b889bdc2633
SHASUM=b543d1c73267bc029f0fc57f468f8fcac27dd20f
SHAALG=1
REPO=
...
CONFIG_FLAGS="--with-libpng=/usr/lib/libpng.so"
MAKE_FLAGS=""
TEST=
TEST_FLAGS="-k"
INSTALL="install"
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# Fix improperly linked automake tools and ltmail from libtool
for file in 'install-sh' compile 'config.sub' 'config.guess' \
        depcomp missing 'test-driver'; do
    ln -sfv /usr/share/automake-*/${file} ${SRCDIR}/
done
ln -sfv /usr/share/libtool/build-aux/ltmain.sh ${SRCDIR}/
# Recreate configuration for system tools
autoreconf ${SRCDIR}
#
# Configure
#^^^^^^^^^^^
