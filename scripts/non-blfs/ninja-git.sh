...
DL_URL=
DL_ALT=
REPO=git://github.com/martine/ninja.git
# VCS=[git,hg,svn,...]
VCS=git
BRANCH=release
# Prepare sources
...
#CMAKE_PARALLEL=1
CONFIG_FLAGS="--bootstrap"
MAKE=""
MAKE_FLAGS=""
TEST=
TEST_FLAGS="-k"
INSTALL="install"
INSTALL_FLAGS_PSYS="-v -Dm755 -o root -g root"
#
...
#
# This package doesn't use make of any kind or in any way - it's python!
#${MAKE} ${MAKE_FLAGS}
#
# Test:
if [ $TEST ]; then
...
#
# Install binary
as_root ${INSTALL} ${INSTALL_FLAGS_PSYS} ninja /usr/bin/
# Bash completion
as_root mkdir -pv /etc/bash_completion.d
as_root cp -fv misc/bash-completion /etc/bash_completion.d/ninja
as_root rm -f misc/bash-completion
# Zsh completion
as_root mkdir -pv /etc/zsh_completion.d
as_root cp -fv misc/zsh-completion /etc/zsh_completion.d/ninja
as_root rm -f misc/zsh-completion
# Vim syntax
mkdir -pv ${HOME}/.vim/after/syntax
mkdir -pv ${HOME}/.vim/syntax
cp -v misc/ninja.vim ${HOME}/.vim/syntax/
cp -v misc/ninja.vim ${HOME}/.vim/after/syntax/
as_root rm -f misc/ninja.vim
# Emacs ninja-mode
mkdir -pv ${HOME}/.config/emacs
cp -v misc/ninja-mode.el ${HOME}/.config/emacs
as_root rm -f misc/ninja-mode.el
# Put the rest in local data folder
mv -v misc ${HOME}/.local/ninja
#
popd
