...
ARCHIVE=tar.gz
MD5=
SHA1=87e6103890a3e85e5bbba8449e8ae680fb8534fa
#
WORKING_DIR=$PWD
SRCDIR=${WORKING_DIR}/${PROG}-${VERSION}
#
# Downloads; obtain and verify package(s)
DL_URL=https://weechat.org/files/src
DL_ALT=
REPO=
# VCS=[git,hg,svn,...]
VCS=
...
#./autogen.sh
#
# Set the remaining cmake options
# Python
if [ ${PYVER} ]; then
    if [ ${PYVER:0:1} == 3 ]; then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DENABLE_PYTHON3=ON"
    elif [ ${PYVER:0:1} == 2 ]; then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DENABLE_PYTHON3=OFF"
    else
        # Suitable python version not given!
        CONFIG_FLAGS="${CONFIG_FLAGS} -DENABLE_PYTHON=OFF -DENABLE_PYTHON3=OFF"
    fi
