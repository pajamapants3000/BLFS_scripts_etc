...
# Options
#
# If using a system that does not support 64-bit numbers (or just long long?)
#L32BIT=1
#
# Preparation
#*************
...
DL_URL=
DL_ALT=
REPO=https://github.com/LuaDist/toluapp.git
# VCS=[git,hg,svn,...]
VCS=git
BRANCH=master
...
if ((${L32BIT})); then
    sed -i "s/CCFLAGS = .*/& '-DLUA_32BITS',/" ./config_linux.py
fi
#
if [ "x${CONFIGURE:$((${#CONFIGURE}-5)):5}" = "xcmake" ]; then
    if ((${CMAKE_PARALLEL})); then
