...
VERSION=0.6.2
ARCHIVE=tar.gz
MD5=8bfbbdb16cf88821c8dacd5165590fd2
#
WORKING_DIR=$PWD
SRCDIR=${WORKING_DIR}/${PROG}-${VERSION}
#
# Downloads; obtain and verify package(s)
DL_URL=http://herbstluftwm.org/tarballs
DL_ALT=
# Prepare sources
...
#
mkdir -pv ${HOME}/.config/herbstluftwm/scripts &> dircheck ||
        grep exists dircheck && rm -f dircheck
mkdir -pv ${HOME}/.config/herbstluftwm/layouts &> dircheck ||
        grep exists dircheck && rm -f dircheck
mkdir -pv ${HOME}/.local/herbstluftwm &> dircheck ||
        grep exists dircheck && rm -f dircheck
cp -Rnv share/* ${HOME}/.config/herbstluftwm
chown -Rv tommy:tommy ${HOME}/.config/herbstluftwm
chmod -Rv 755 ${HOME}/.config/herbstluftwm
sudo install -v -Dm755 -o root -g root \
        ${HOME}/.config/herbstluftwm/herbstluftwm.desktop \
        /usr/share/applications
sudo install -v -Dm755 -o root -g root \
        ${HOME}/.config/herbstluftwm/herbstluftwm-completion \
        /etc/bash_completion.d/
mv -v ${HOME}/.{config,local}/herbstluftwm/herbstluftwm.xpm
mv -v ${HOME}/.{config,local}/herbstluftwm/herbstluftwm.svg
#
popd
