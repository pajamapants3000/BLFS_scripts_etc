...
# Options
#********
# Uncomment to build with BLAS if available
BLAS=1
# Uncomment to build with Coin3D if available
COIN3D=1
# Uncomment to build with ESBTL if available
ESBTL=1
# Uncomment to build with Eigen3 if available
EIGEN3=1
# Uncomment to build with GMP if available
GMP=1
# Uncomment to build with GMPXX if available
GMPXX=1
# Uncomment to build with IPE if available
IPE=1
# Uncomment to build with LAPACK if available
LAPACK=1
# Uncomment to build with LEDA if available
LEDA=1
# Uncomment to build with MPFI if available
MPFI=1
# Uncomment to build with MPFR if available
MPFR=1
# Uncomment to build with NTL if available
NTL=1
# Uncomment to build with OpenGL if available
OPENGL=1
# Uncomment to build with OpenNL if available
OPENNL=1
# Uncomment to build with QGLViewer if available
QGLVIEWER=1
# Uncomment to build with RS if available
RS=1
# Uncomment to build with RS3 if available
RS3=1
# Uncomment to build with ZLIB if available
ZLIB=1
# Uncomment to build with demos
DEMOS=1
# Uncomment to build with examples
EXAMPLES=1
...
DL_URL=https://github.com/CGAL/cgal/releases/download/releases%2F
DL_ALT=
MD5=ee31343dbc4bf7b5b7501ec1650e9233
SHASUM=
SHAALG=1
REPO=
...
CONFIG_FLAGS=""
if ((ZLIB)); then 
    CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_ZLIB=ON"
else
   CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_ZLIB=OFF"
fi
if ((DEMOS)); then
    CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_demos=ON"
else
    CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_demos=OFF"
fi
if ((EXAMPLES)); then
    CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_examples=ON"
else
    CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_examples=OFF"
fi
if ((MPFR)); then
    CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_MPFR=ON"
else
    CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_MPFR=OFF"
fi
if ((GMP)); then
    CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_GMP=ON"
else
    CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_GMP=OFF"
fi
if ((GMPXX)); then
    CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_GMPXX=ON"
else
    CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_GMPXX=OFF"
fi
if ((OPENGL)); then
    CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_OpenGL=ON"
else
    CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_OpenGL=OFF"
fi
if (cat /list-$CHRISTENED-$SURNAME | grep '^blas-' > /dev/null);
then
    if ((BLAS)); then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_BLAS=ON"
    else
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_BLAS=OFF"
    fi
fi
if (cat /list-$CHRISTENED-$SURNAME | grep '^coin3d-' > /dev/null);
then
    if ((COIN3D)); then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_Coin3D=ON"
    else
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_Coin3D=OFF"
    fi
fi
if (cat /list-$CHRISTENED-$SURNAME | grep '^esbtl-' > /dev/null);
then
    if ((ESBTL)); then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_ESBTL=ON"
    else
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_ESBTL=OFF"
    fi
fi
if (cat /list-$CHRISTENED-$SURNAME | grep '^eigen-3' > /dev/null);
then
    if ((EIGEN3)); then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_Eigen3=ON"
    else
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_Eigen3=OFF"
    fi
fi
if (cat /list-$CHRISTENED-$SURNAME | grep '^ipe-' > /dev/null);
then
    if ((IPE)); then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_IPE=ON"
    else
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_IPE=OFF"
    fi
fi

if (cat /list-$CHRISTENED-$SURNAME | grep '^lapack-' > /dev/null);
then
    if ((LAPACK)); then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_LAPACK=ON"
    else
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_LAPACK=OFF"
    fi
fi
if (cat /list-$CHRISTENED-$SURNAME | grep '^leda-' > /dev/null);
then
    if ((LEDA)); then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_LEDA=ON"
    else
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_LEDA=OFF"
    fi
fi

if (cat /list-$CHRISTENED-$SURNAME | grep '^mpfi-' > /dev/null);
then
    if ((MPFI)); then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_MPFI=ON"
    else
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_MPFI=OFF"
    fi
fi
if (cat /list-$CHRISTENED-$SURNAME | grep '^ntl-' > /dev/null);
then
    if ((NTL)); then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_NTL=ON"
    else
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_NTL=OFF"
    fi
fi
if (cat /list-$CHRISTENED-$SURNAME | grep '^opennl' > /dev/null);
then
    if ((OPENNL)); then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_OpenNL=ON"
    else
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_OpenNL=OFF"
    fi
fi
if (cat /list-$CHRISTENED-$SURNAME | grep '^libqglviewer-' > /dev/null);
then
    if ((QGLVIEWER)); then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_QGLViewer=ON"
    else
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_QGLViewer=OFF"
    fi
fi
if (cat /list-$CHRISTENED-$SURNAME | grep '^librs-' > /dev/null);
then
    if ((RS)); then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_RS=ON"
    else
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_RS=OFF"
    fi
    if ((RS3)); then
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_RS3=ON"
    else
        CONFIG_FLAGS="${CONFIG_FLAGS} -DWITH_RS3=OFF"
    fi
fi
MAKE_FLAGS=""
TEST=
