...
#
# Dependencies
# See configure --help output or INSTALL file in source root.
# This will work best with a "list" to install dependencies.
#
# Options
#********
...
SHORTVER=${VERSION%\.*}
ARCHIVE=tar.xz
MD5=da4bc3e47afd0faf9e7a67168e012102
#
WORKING_DIR=$PWD
SRCDIR=${WORKING_DIR}/${PROG}-${VERSION}
#
# Downloads; obtain and verify package(s)
DL_URL=http://www.musicpd.org/download
DL_ALT=
REPO=
# VCS=[git,hg,svn,...]
VCS=
...
if [ ${VCS} ]; then
    ...
    if ! [ -f ${PROG}-${VERSION}.${ARCHIVE} ]; then
        wget ${DL_URL}/${PROG}/${SHORTVER}/${PROG}-${VERSION}.${ARCHIVE} \
