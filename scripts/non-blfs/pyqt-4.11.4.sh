...
DL_ALT=
MD5=2fe8265b2ae2fc593241c2c84d09d481
SHASUM=5c489e655279f85d47c8a275d97a12dfa0c8fd87
SHAALG=1
REPO=
...
CONFIGURE="python2 ${SRCDIR}/configure-ng.py"
#
# Flags
#*******
...
if ((_OPT_INSTALL)); then
    CONFIG_FLAGS="INCPATH+=/usr/include/phonon     \
                  INCPATH+=/usr/include/python2.7  \
                --confirm-license --bindir=/opt/pyqt4/bin --destdir=/opt/pyqt4"
else
    CONFIG_FLAGS="INCPATH+=/usr/include/phonon     \
                  INCPATH+=/usr/include/python2.7  \
                  --confirm-license"
...
#
    ...
    if ! [ -f ${WORKING_DIR}/${PROG}-${VERSION}.${ARCHIVE} ]; then
        wget ${DL_URL}/${PROG_ALT}4/${PROG_ALT}-${VERSION}/${PROG_ALT}-x11-gpl-${VERSION}.${ARCHIVE} \
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
[ $QT_INSTALL_PLUGINS ] || export QT_INSTALL_PLUGINS="${QT4DIR}/plugins"
[ $QT_INSTALL_DATA ]    || export QT_INSTALL_DATA="${QT4DIR}"
#
# Configure
#^^^^^^^^^^^
if [ "${CONFIGURE}" ]; then
...
if ((_OPT_INSTALL)); then
    ...
    #as_root lincon -a /etc/profile.d/pyqt4.sh
    # but for now...
    as_root ln -sfv \
            ${HOME}/.lincon/conf_fs/etc/profile.d/pyqt4.sh /etc/profile.d/pyqt4.sh
    # then
    as_root ln -sfv pyqt4.sh /etc/profile.d/pyqt.sh
fi
