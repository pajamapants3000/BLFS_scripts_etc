...
# Options
#********
# Uncomment to use Python 3 if both are available
USE_PY3=1
# Uncomment to prevent building of static libraries (shared only)
NOSTATIC=1
# Uncomment for bigger test - probably won't want to ever do
#BIGTEST=1
...
DL_URL=http://downloads.sourceforge.net/project
DL_ALT=
...
if ((USE_PY3)) &&
        (cat /list-${CHRISTENED}-${SURNAME} | grep '^python-2' > /dev/null) &&
        (cat /list-${CHRISTENED}-${SURNAME} | grep '^python-2' > /dev/null);
then
    CONFIG_FLAGS="${CONFIG_FLAGS} --with-python=python3"
fi
MAKE_FLAGS="stage threading=multi"
if ((NOSTATIC)); then
    MAKE_FLAGS="${MAKE_FLAGS} link=shared"
fi
PYTHON2="python2"
TEST="test_all.py"
TEST_FLAGS=""
...
# Boost
#^^^^^^^^^
elif [ "x${CONFIGURE:$((${#CONFIGURE}-13)):13}" = "x/bootstrap.sh" ]; then
    CONFIG_FLAGS="${CONFIG_FLAGS}"
    MAKE="${SRCDIR}/b2"
# Default
#^^^^^^^^^
...
#
    ...
    if ! [ -f ${WORKING_DIR}/${PROG}-${VERSION}.${ARCHIVE} ]; then
        wget ${DL_URL}/${PROG_ALT}/${PROG_ALT}/${VERSION}/${PROG_ALT}_${V_E_R}.${ARCHIVE} \
...
if ((USE_PY3)) &&
        (cat /list-${CHRISTENED}-${SURNAME} | grep '^python-2' > /dev/null) &&
        (cat /list-${CHRISTENED}-${SURNAME} | grep '^python-3' > /dev/null);
then
...
# More complete test (120SBU, up to 40GB)
if ((BIGTEST)); then
    pushd status
    ../b2
    popd
...
# Install
#^^^^^^^^^
as_root ${MAKE} ${INSTALL} ${INSTALL_FLAGS}
#
