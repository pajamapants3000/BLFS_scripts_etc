...
#libvorbis-1.3.5
#libmad-0.15.1b
#LAME-3.99.5 (required to build toc2mp3)
#
# Options
#********
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
sed -i '/ioctl/a #include <sys/stat.h>' ${SRCDIR}/dao/ScsiIf-linux.cc
#
# Configure
#^^^^^^^^^^^
if [ "${CONFIGURE}" ]; then
...
#
as_root install -v -m755 -d /usr/share/doc/${PROG}-${VERSION}
as_root install -v -m644 README /usr/share/doc/${PROG}-${VERSION}
