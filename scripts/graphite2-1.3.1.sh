...
# Installation script for graphite2-1.3.1
DATE=$(date +%Y%m%d)
TIME=$(date +%H%M%S)
#
...
#python-2.7.10
#silgraphite
#harfbuzz-1.0.2 (circular: graphite2 -> harfbuzz -> graphite2)
# Optional - Documentation: asciidoc, doxygen-1.8.10 and texlive-20150521 (or install-tl-unx)
# without python2 module FontTools; some tests will fail
# Optional (at runtime)
...
DL_URL=http://downloads.sourceforge.net/silgraphite
DL_ALT=
# Prepare sources
...
if ((${BLDDOC})) &&
        (cat /list-${CHRISTENED}-${SURNAME} | grep "doxygen-" > /dev/null) &&
        (cat /list-${CHRISTENED}-${SURNAME} | grep "asciidoc-" > /dev/null) &&
...
#
if ((${BLDDOC})); then
as_root install -v -d -m755 /usr/share/doc/graphite2-1.3.1/api
as_root cp      -v -r -f    doc/doxygen/{html,latex/refman.pdf} \
        /usr/share/doc/graphite2-1.3.1/api
as_root cp      -v -r -f    doc/{GTF,manual}.html \
