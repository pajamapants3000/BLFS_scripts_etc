...
# Options
#********
# Uncomment to build and install API documentation
APIDOCS=1
...
CONFIG_FLAGS="--enable-xft       \
              --enable-shared    \
              --enable-threads   \
              --enable-largefile \
              --enable-xinerama"
MAKE_FLAGS=""
MAKE_APIDOC_FLAGS="-C documentation"
TEST=
TEST_FLAGS="-k"
INSTALL="install"
INSTALL_OPT="install-linux"
INSTALL_OPT_FLAGS_TEST="-C test docdir=${DOCDER}"
INSTALL_OPT_FLAGS_DOC="-C documentation docdir=${DOCDER}"
INSTALL_FLAGS="docdir=${DOCDER}"
#
...
#^^^^^^^^^^^
    ...
    [ "${CFG_LIBDIR_FLAG}" ]        || CFG_LIBDIR_FLAG="--libdir"
    [ "${CFG_INCLUDEDIR_FLAG}" ]    || CFG_INCLUDEDIR_FLAG="--includedir"
    ...
    [[ ${LIBDER} ]] &&
            CONFIG_FLAGS="${CONFIG_FLAGS} ${CFG_LIBDIR_FLAG}=${LIBDER}" || (exit 0)
    [[ ${INKLOODDER} ]] &&
            CONFIG_FLAGS="${CONFIG_FLAGS} ${CFG_INCLUDEDIR_FLAG}=${INKLOODDER}" || (exit 0)
    [[ ${DOCDER} ]] &&
...
#
    ...
    if ! [ -f ${WORKING_DIR}/${PROG}-${VERSION}.${ARCHIVE} ]; then
        wget ${DL_URL}/${VERSION}/${PROG_ALT}-${VERSION}-source.${ARCHIVE} \
            -O ${WORKING_DIR}/${PROG}-${VERSION}.${ARCHIVE} || FAIL_DL=1
        # FTP/alt Download:
        if (($FAIL_DL)) && [ "$DL_ALT" ]; then
            wget ${DL_ALT}/${VERSION}/${PROG_ALT}-${VERSION}-source.${ARCHIVE} \
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
sed -i -e '/cat./d' ${SRCDIR}/documentation/Makefile
#
# Configure
#^^^^^^^^^^^
if [ "${CONFIGURE}" ]; then
...
if ! ((BUILD_ONLY)); then
    ...
    if ((APIDOCS)); then
        as_root ${MAKE} ${INSTALL_OPT_FLAGS_TEST} ${INSTALL_OPT}
        as_root ${MAKE} ${INSTALL_OPT_FLAGS_DOC} ${INSTALL_OPT}
    fi
fi
#
