...
DL_URL=https://bitbucket.org/multicoreware/x265/downloads/x265_${VERSION}.${ARCHIVE}
DL_ALT=
DL_PATCH=http://www.linuxfromscratch.org/patches/blfs/svn/x265_2.1-enable_static-1.patch
MD5=2d9cb183d2675dfb325abdedd2424bfa
SHASUM=
SHAALG=1
REPO=
...
BLFS_PATCH=x265_2.1-enable_static-1.patch
if [ ${PATCH} ]; then
    [ -f ${PATCHDIR}/${PATCH} ] ||
...
CONFIG_FLAGS="-DENABLE_STATIC=OFF"
MAKE_FLAGS=""
TEST=
TEST_FLAGS=""
INSTALL="install"
...
if [ "x${CONFIGURE:$((${#CONFIGURE}-5)):5}" = "xcmake" ]; then
    [ "${CMAKE_SRC_ROOT}" ] || CMAKE_SRC_ROOT=${SRCDIR}/source
...
#
    # Download patch
    #****************
    if ! [ -f ${WORKING_DIR}/${BLFS_PATCH} ]; then
        wget ${DL_PATCH} -O ${WORKING_DIR}/${BLFS_PATCH}
    fi
    # Verify package
    #****************
    if [ "${SHASUM}" ]; then
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^
[ "${BLFS_PATCH}" ] && patch -Np1 < ${WORKING_DIR}/${BLFS_PATCH} || (exit 0)
...
if ! ((RETAIN_DL)); then
    ...
    [ -e ${WORKING_DIR}/${BLFS_PATCH} ] &&
            as_root rm ${WORKING_DIR}/${BLFS_PATCH}
fi
#
