...
# Options
#********
# Emacs version - Default NO
PINENTRY_EMACS=0
# Qt version - Default YES
PINENTRY_QT=1
# GTk version - Default YES; pinentry symlinks to pinentry-gtk-2
PINENTRY_GTK=1
# Gnome version - Default YES
PINENTRY_GNOME=1
#
...
DL_URL=ftp://ftp.gnupg.org/gcrypt
DL_ALT=
...
CONFIG_FLAGS=""
if ((PINENTRY_EMACS)) &&
    (cat /list-${CHRISTENED}-${SURNAME} | grep '^emacs-' > /dev/null); then
    CONFIG_FLAGS="${CONFIG_FLAGS} --enable-emacs=yes"
else
    CONFIG_FLAGS="${CONFIG_FLAGS} --enable-emacs=no"
fi
if ((PINENTRY_QT)) &&
    (cat /list-${CHRISTENED}-${SURNAME} | grep '^qt-5' > /dev/null); then
    CONFIG_FLAGS="${CONFIG_FLAGS} --enable-pinentry-qt=yes"
else
    CONFIG_FLAGS="${CONFIG_FLAGS} --enable-pinentry-qt=no"
fi
if ((PINENTRY_GTK)) &&
    (cat /list-${CHRISTENED}-${SURNAME} | grep '^gtk+-2' > /dev/null); then
    CONFIG_FLAGS="${CONFIG_FLAGS} --enable-pinentry-gtk2=yes"
else
    CONFIG_FLAGS="${CONFIG_FLAGS} --enable-pinentry-gtk2=no"
fi
if ((PINENTRY_GNOME)) &&
    (cat /list-${CHRISTENED}-${SURNAME} | grep '^gtk+-3' > /dev/null); then
    CONFIG_FLAGS="${CONFIG_FLAGS} --enable-pinentry-gnome3=yes"
else
    CONFIG_FLAGS="${CONFIG_FLAGS} --enable-pinentry-gnome3=no"
fi
MAKE_FLAGS=""
TEST=
TEST_FLAGS="-k"
