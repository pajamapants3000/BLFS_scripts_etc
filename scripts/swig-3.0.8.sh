...
CONFIG_FLAGS="--without-clisp --without-maximum-compile-warnings"
MAKE="make"
MAKE_FLAGS=""
TEST=check
TEST_FLAGS="-k"
if (cat /list-${CHRISTENED}-${SURNAME} | grep "^python-2" > /dev/null) &&
   (cat /list-${CHRISTENED}-${SURNAME} | grep "^python-3" > /dev/null); then
    TEST_PY3EX=check-python-examples
    TEST_PY3TS=check-python-test-suite
fi
INSTALL="install"
INSTALL_FLAGS=""
#
...
#
# Special test for Python3 if both are installed
if [ "${TEST_PY3EX}" ]; then
    PY3=1 ${MAKE} ${TEST_FLAGS} ${TEST_PY3EX} 2>&1 | \
        tee ${WORKING_DIR}/logs/${PROG}-${VERSION}-${DATE}.py3ex.check || (exit 0)
    STAT=${PIPESTATUS[0]}
    if (( STAT )); then
        echo "Some tests failed; log in ../logs/$(basename $0)-log"
        echo "Pull up another terminal and check the output"
...
#
# Other special test for Python3 if both are installed
if [ "${TEST_PY3TS}" ]; then
    [ -d ${WORKING_DIR}/logs ] || mkdir -v ${WORKING_DIR}/logs
    PY3=1 ${MAKE} ${TEST_FLAGS} ${TEST_PY3TS} 2>&1 | \
        tee ${WORKING_DIR}/logs/${PROG}-${VERSION}-${DATE}.py3ts.check || (exit 0)
    STAT=${PIPESTATUS[0]}
    if (( STAT )); then
        echo "Some tests failed; log in ../logs/$(basename $0)-log"
        echo "Pull up another terminal and check the output"
