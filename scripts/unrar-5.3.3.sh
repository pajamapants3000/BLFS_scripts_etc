...
VERSION=5.3.3
ARCHIVE=tar.gz
MD5=e19f3f4fd58e85c1ecdcd0ef006998a9
SHA1=
#
WORKING_DIR=$PWD
...
DL_URL=http://www.rarlab.com/rar
DL_ALT=
REPO=
# VCS=[git,hg,svn,...]
#VCS=${VERSION}
...
TEST=
TEST_FLAGS="-k"
INSTALL="install -v -m755 unrar /usr/bin"
INSTALL_FLAGS=""
#
...
if [ "${VCS}" ]; then
    ...
    if ! [ -f ${PROG}-${VERSION}.${ARCHIVE} ]; then
        wget ${DL_URL}/${PROG}src-${VERSION}.${ARCHIVE} \
            -O ${PROG}-${VERSION}.${ARCHIVE} || FAIL_DL=1
        # FTP/alt Download:
        if (($FAIL_DL)) && [ "$DL_ALT" ]; then
            wget ${DL_ALT}/${PROG}src-${VERSION}.${ARCHIVE} \
