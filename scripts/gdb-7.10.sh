...
CONFIG_FLAGS="--with-system-readline"
MAKE="make"
MAKE_FLAGS=""
#TEST="site.exp"
TEST_FLAGS="-k"
...
${MAKE} ${MAKE_FLAGS}
#
# with doxygen, build optional docs
if (($DOXYDOCS)) && (cat /list-${CHRISTENED}-${SURNAME} | \
    grep "doxygen-" > /dev/null); then
    ${MAKE} -C gdb/doc doxy
else
    DOXYDOCS=0
fi
# Test:
# There are apparently a lot of issues with this test suite; not sure which
#+command needs to catch failures, so I wrapped both.
if [ $TEST ]; then
    pushd gdb/testsuite
...
#
if (($DOXYDOCS)); then
    as_root install -d /usr/share/doc/${PROG}-${VERSION}
    as_root rm -rf gdb/doc/doxy/xml
    as_root cp -Rv gdb/doc/doxy /usr/share/doc/${PROG}-${VERSION}
fi
#
popd
