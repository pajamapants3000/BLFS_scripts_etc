...
#curl-7.51.0
#dbus_glib-0.108
#ghostscript-9.20 (with libgs installed)
#gvfs-1.30.0
#iso_codes-3.71
#jasper-2.0.0
#little_cms-1.19
...
#poppler-0.49.0
#mta
#webkitgtk+-2.4.11 (required for the help_plugin)
#gtk_doc-1.25
#libwmf
#pngnq
#pngcrush  
...
CONFIG_FLAGS="--without-gfvs"
if ! (cat /list-Lilu-lfs-20150723 | grep "^pygtk-" > /dev/null); then
    ((NOPYTHONOK)) && CONFIG_FLAGS="${CONFIG_FLAGS} --disable-python" ||
            exit 1
fi
if (cat /list-Lilu-lfs-20150723 | grep "^gtk_doc-" > /dev/null); then
    ((GTKDOC)) && CONFIG_FLAGS="${CONFIG_FLAGS} --enable-gtk-doc" || (exit 0)
fi
MAKE_FLAGS=""
# test requires x-window terminal
TEST="check"
TEST_FLAGS=""
INSTALL="install"
INSTALL_FLAGS=""
...
#
    ...
    if ! [ -f ${WORKING_DIR}/${PROG}-${VERSION}.${ARCHIVE} ]; then
        wget ${DL_URL}/${PROG_ALT}/v${VER}/${PROG_ALT}-${VERSION}.${ARCHIVE} \
...
#
# Install help
${SCRIPTDIR}/gimp_help-${HELP_VERSION}.sh
#
