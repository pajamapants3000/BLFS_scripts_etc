...
# Installation script for wpa_supplicant-2.4
DATE=$(date +%Y%m%d)
TIME=$(date +%H%M%S)
#
...
DBUS=1
QTGUI=1
# Options are: static, dhcp, dhcpcd
ACCESS="static"
#
# Preparation
#*************
...
VERSION=2.4
ARCHIVE=tar.gz
MD5=f0037dbe03897dcaf2ad2722e659095d
#
WORKING_DIR=$PWD
SRCDIR=${WORKING_DIR}/${PROG}-${VERSION}
#
# Downloads; obtain and verify package(s)
DL_URL=http://hostap.epitest.fi/releases
DL_ALT=
# Prepare sources
...
CONFIG_FLAGS=""
MAKE="make"
MAKE_FLAGS="BINDIR=${BINDER} LIBDIR=${LIBDER}"
TEST=
TEST_FLAGS="-k"
INSTALL="install"
INSTALL_FLAGS=""
...
#
cp -v ../files/wpa_supplicant_config wpa_supplicant/.config
if (cat /list-${CHRISTENED}-${SURNAME} | grep "dbus-" > /dev/null) &&
        (cat /list-${CHRISTENED}-${SURNAME} | grep "libxml2-" > /dev/null); then
...
#
if (($QTGUI)) &&
        (cat /list-${CHRISTENED}-${SURNAME} | grep "qt-4" > /dev/null); then
    pushd wpa_gui-qt4
    qmake wpa_gui.pro
    ${MAKE}
    popd
...
if (cat /list-${CHRISTENED}-${SURNAME} | \
        grep "desktop_file_utils-" > /dev/null); then
...
# Configuration
#***************
# (note wifi0 and wlan0 are variable)
#
# To set up a password protected connection
#wpa_passphrase SSID SECRET_PASSWORD > /etc/sysconfig/wpa_supplicant-wifi0.conf
#
# To connect to a wifi network that isn't password protected, add
#network={
#  ssid="Some-SSID"
#    key_mgmt=NONE
#}
...
#
# Finally, to connect to a wireless access point
#ifup wifi0
#
###################################################
#
