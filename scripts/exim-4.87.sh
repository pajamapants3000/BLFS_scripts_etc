...
# Options
#********
# LFS: use basic defaults from LFS; otherwise use pre-made makefiles
#LFS=1
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
if ((LFS)); then    # use basic LFS default
    sed -e 's,^BIN_DIR.*$,BIN_DIRECTORY=/usr/sbin,' \
            -e 's,^CONF.*$,CONFIGURE_FILE=/etc/exim.conf,' \
            -e 's,^EXIM_USER.*$,EXIM_USER=exim,' \
            -e 's,^EXIM_MONITOR,#EXIM_MONITOR,' \
            ${SRCDIR}/src/EDITME > ${SRCDIR}/Local/Makefile
    printf "USE_GDBM = yes\nDBMLIB = -lgdbm\n" >> ${SRCDIR}/Local/Makefile
else                # use pre-made config files
    cp -R ${BLFSDIR}/files/${PROG}-${VERSION} ${SRCDIR}/Local
fi
#
# Build
#^^^^^^^
